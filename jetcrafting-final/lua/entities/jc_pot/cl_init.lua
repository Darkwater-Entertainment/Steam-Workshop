--[[
 - cl_init.lua is the client file for the Pot entity.
 - 
 - @author Kyle James
...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < JC_UI_RANGE then
		...
		cam.Start3D2D(pos + ang:Up() * 8, ang, 0.055);
			...
			draw.SimpleText("Amphetamine", "TerminalFont", 0, -56, JC_TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
			...
			draw.SimpleText("Spoiled Meat (" .. self:GetNWInt("spoiledMeat") .. ")", "TerminalFont", -101, 38, JC_TEXT_COLOR, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER);
			draw.SimpleText("Protein Extract (" .. self:GetNWInt("proteinExtract") .. ")", "TerminalFont", -101, 68, JC_TEXT_COLOR, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER);				
