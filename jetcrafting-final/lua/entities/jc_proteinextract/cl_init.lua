--[[
 - cl_init.lua is the client file for the Protein Extract entity.
 - 
 - @author Kyle James
...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < JC_UI_RANGE then
		...
		cam.Start3D2D(pos+ang:Up()*3.35, ang, 0.04);
			...
			draw.SimpleText("Protein", "TerminalFont", 0, -675, JC_TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
			draw.SimpleText("Extract", "TerminalFont", 0, -650, JC_TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
