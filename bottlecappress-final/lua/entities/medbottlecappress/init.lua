--[[
 - init.lua is the server file for the Medium Bottlecap Press entity.
 - 
 - @author Kyle James
...
function ENT:Think()
	...
	if CurTime() > self.timer + MEDBP_INTERVAL then
		
        -- Reset the timer
		self.timer = CurTime();

        -- If the number of stored caps is fewer than the maximum
        if self:GetMoneyAmount() < MEDBP_MAX or MEDBP_MAX <= 0 then

            -- Increment the caps stored
            self:SetMoneyAmount(self:GetMoneyAmount() + MEDBP_INCREMENT);
...
function ENT:Use(act)
    ...
    if gmod.GetGamemode().Name == "DarkRP" then
		...
		if IsValid(act) and money > 0 then
			...
			DarkRP.notify(act, NOTIFY_GENERIC, 4, "You collected " .. money .. " caps from the Bottlecap Press v2.");
...
function ENT:OnTakeDamage(dmg)
    ...
    -- If destructible
    if MEDBP_HEALTH > 0 then

        -- Make the entity take damage
        self:TakePhysicsDamage(dmg);

        -- Set the damage the entity has taken
        self.damage = (self.damage or MEDBP_HEALTH) - dmg:GetDamage();
...
function ENT:Destruct()
    ...
    if IsValid(self:Getowning_ent()) then
        ...
        DarkRP.notify(self:Getowning_ent(), NOTIFY_ERROR, 4, "Your Medium Bottlecap Press exploded!");
