--[[
 - init.lua is the server file for the Big Bottlecap Press entity.
 - 
 - @author Kyle James
...
function ENT:Initialize()
	...
	--self:SetModel("models/props_fallout/industrymachine02.mdl"); -- OLD MODEL: https://steamcommunity.com/sharedfiles/filedetails/?id=386930987
	self:SetModel("models/props_wasteland/laundry_washer003.mdl"); -- "NEW" MODEL: More user-friendly (fewer workshop items to install)
...
function ENT:Think()
	...
	if CurTime() > self.timer + BIGBP_INTERVAL then
		
		-- Reset the timer
		self.timer = CurTime();

		-- If the number of stored caps is fewer than the maximum
        if self:GetMoneyAmount() < BIGBP_MAX or BIGBP_MAX <= 0 then

            -- Increment the caps stored
            self:SetMoneyAmount(self:GetMoneyAmount() + BIGBP_INCREMENT);
...
function ENT:Use(act)
	...
	if gmod.GetGamemode().Name == "DarkRP" then
		...
		if IsValid(act) and money > 0 then
			...
			DarkRP.notify(act, NOTIFY_GENERIC, 4, "You collected " .. money .. " caps from the Bottlecap Press v3.");
...
function ENT:OnTakeDamage(dmg)
	...
	-- If destructible
	if BIGBP_HEALTH > 0 then

		-- Make the entity take damage
		self:TakePhysicsDamage(dmg);

		-- Set the damage the entity has taken
		self.damage = (self.damage or BIGBP_HEALTH) - dmg:GetDamage();
...
function ENT:Destruct()
    ...
    if IsValid(self:Getowning_ent()) then
        ...
        DarkRP.notify(self:Getowning_ent(), NOTIFY_ERROR, 4, "Your Big Bottlecap Press exploded!");
