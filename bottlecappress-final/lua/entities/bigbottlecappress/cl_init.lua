--[[
 - cl_init.lua is the client file for the Big Bottlecap Press entity.
 - 
 - @author Kyle James
...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < BP_UI_RANGE then
		...
		--[[ OLD MODEL: https://steamcommunity.com/sharedfiles/filedetails/?id=386930987
		...
		-- "NEW" MODEL: More user-friendly (fewer workshop items to install)
		-- Set appropriate positions for UI
		t = Vector(-15, 54, -24);
		t:Rotate(self:GetAngles());
		pos = pos + t;
		...
		ang:RotateAroundAxis(self:GetAngles():Forward(), 135);
		...
		cam.Start3D2D(pos, ang, 0.1);
			...
			draw.SimpleText("Bottle Cap Press v3", "MoneyFont", 0, -675, BP_TEXT_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER);
